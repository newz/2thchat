	...
	if(preg_match_all('/@(.*?)(\s|\z)/',$attextn,$atmatch)) {
        foreach ($atmatch[1] as $atvalue) {
			...
			if($atuser&&!empty($atuser['name'])&&$config['namemode']==2){
				$attext = paddslashes('<a class="nzca" onclick="nzAt(\''.$atvalue.'\');"><font color="'.$atuser['color'].'">'.htmlspecialchars_decode($atuser['name']).'</font></a> ');
			}else if($atuser){
				$attext = paddslashes('<a class="nzca" onclick="nzAt(\''.$atvalue.'\');"><font color="'.$atuser['color'].'">'.$atvalue.'</font></a> ');
			}else{
				$attext = '@'.$atvalue;
			}
...
function getquota($quota){
	...
	if($quo = DB::query("SELECT uid,text FROM ".DB::table('newz_data')." WHERE id='".$quota."'"))
	{
		...
		$atuser = DB::fetch($atuser);
		if($atuser&&!empty($atuser['name'])&&$config['namemode']==2){
			$attext = paddslashes('<a class="nzca" onclick="nzAt(\''.$atuser['username'].'\');"><font color="'.$atuser['color'].'">'.htmlspecialchars_decode($atuser['name']).'</font></a>');
		}else if($atuser){
			$attext = paddslashes('<a class="nzca" onclick="nzAt(\''.$atuser['username'].'\');"><font color="'.$atuser['color'].'">'.$atuser['username'].'</font></a>');
		}else{
			$attext = '???';
		}
...
<span><a href="home.php?mod=space&uid='.$uid_p.'" class="nzca" target="_blank"><font color="'.$color.'">'.$icon.'<span class="nzuname_'.$uid_p.'">'.$username.'</span></font></a> <span id="nzstatus" class="nzustatus_'.$uid_p.'">'.$status.'</span></span><br />
