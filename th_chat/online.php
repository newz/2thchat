...
if ($config['chat_point']) {
    $re = DB::query("SELECT s.uid,s.username,s.groupid,s.lastactivity,g.grouptitle,g.color,n.ban,n.point_total" . ($config['chat_point'] != '9' ? ",p.extcredits{$config['chat_point']} AS point" : "") . " FROM " . DB::table('common_session') . " s LEFT JOIN " . DB::table('common_usergroup') . " g ON s.groupid=g.groupid LEFT JOIN " . DB::table('newz_nick') . " n ON s.uid=n.uid LEFT JOIN " . DB::table('common_member_count') . " p ON s.uid=p.uid WHERE s.uid>0 AND invisible=0 AND action IN (2,127) AND fid=0 AND tid=0");
    if (!empty($config['onlinebot'])) {
        $re2 = DB::query("SELECT s.uid,s.username,s.groupid,g.grouptitle,g.color,n.ban,n.point_total" . ($config['chat_point'] != '9' ? ",p.extcredits{$config['chat_point']} AS point" : "") . " FROM " . DB::table('common_member') . " s LEFT JOIN " . DB::table('common_usergroup') . " g ON s.groupid=g.groupid LEFT JOIN " . DB::table('newz_nick') . " n ON s.uid=n.uid LEFT JOIN " . DB::table('common_member_count') . " p ON s.uid=p.uid WHERE s.uid IN (" . $config['onlinebot'] . ")");
    }
} else {
    $re = DB::query("SELECT s.uid,s.username,s.groupid,s.lastactivity,g.grouptitle,g.color,n.ban,n.point_total FROM " . DB::table('common_session') . " s LEFT JOIN " . DB::table('common_usergroup') . " g ON s.groupid=g.groupid LEFT JOIN " . DB::table('newz_nick') . " n ON s.uid=n.uid WHERE s.uid>0 AND invisible=0 AND action IN (2,127) AND fid=0 AND tid=0");
    if (!empty($config['onlinebot'])) {
        $re2 = DB::query("SELECT s.uid,s.username,s.groupid,g.grouptitle,g.color,n.ban,n.point_total FROM " . DB::table('common_member') . " s LEFT JOIN " . DB::table('common_usergroup') . " g ON s.groupid=g.groupid LEFT JOIN " . DB::table('newz_nick') . " n ON s.uid=n.uid WHERE s.uid IN (" . $config['onlinebot'] . ")");
    }
}
while ($r = DB::fetch($re) or $r = DB::fetch($re2)) {
    ...
    if ($time - $r['lastactivity'] > $timeout) {
		...
		</div><div style="display:inline-block;margin-left:10px;position:relative;top:-4px;height:30px;"><a class="nznametop2 nzat_'.$r['uid'].'" id="nzolpro_' . $r['uid'] . '" onclick="showWindow(\'th_chat_profile\', \'plugin.php?id=th_chat:profile&uid='.$r['uid'].'\');return false;" style="cursor:pointer'.($r['color']?';color:'.$r['color']:'').'">' . ($r['ban']?'<strike>' . $r['username'] . '</strike>':$r['username']) . '</a><br>
