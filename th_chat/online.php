...
// End Show the verified icon when to enabled only
	...
	if($time-$r['lastactivity']>$timeout2)
	{
		...
		$body_onlineex[$r['groupid']] .= '<div id="nzolpro_'.$r['uid'].'_menu" class="nzchatpro" style="display:none;"><img src="'.avatar($r['uid'],'middle',1).'" alt="" onError="this.src=\'uc_server/images/noavatar_middle.gif\';" /><br /><a href="home.php?mod=space&amp;uid='.$r['uid'].'" style="color:'.$r['color'].';" target="_blank" class="nzca" >'.stripslashes($r['nameold']).'</a>'.(($config['verifyicon_onlinelistajax']==1)?''.$thzaa_verify[$r['uid']].'':'').''.($config['chat_point']?'<br />'.lang('plugin/th_chat', 'jdj_th_chat_text_php_61').': '.$r['point']:'').($config['namemode']==1?'<br/><span id="nzstatus" class="nzustatus_'.$r['uid'].'">'.$status.'</span>':'').'<div style="text-align:left;padding-left:20px;margin-bottom:7px;"><img src="source/plugin/th_chat/images/av.gif" align="absmiddle" alt="" /> <a href="home.php?mod=spacecp&ac=avatar">'.lang('plugin/th_chat', 'jdj_th_chat_text_php_36').'</a></div></div>';
	}else{
