...
if($config['chat_point']){
	$re = DB::query("SELECT s.uid,s.username,s.groupid,s.lastactivity,g.color,n.point_total".($config['chat_point']!='9'?",p.extcredits{$config['chat_point']} AS point":"")." FROM ".DB::table('common_session')." s LEFT JOIN ".DB::table('common_usergroup')." g ON s.groupid=g.groupid LEFT JOIN ".DB::table('newz_nick')." n ON s.uid=n.uid LEFT JOIN ".DB::table('common_member_count')." p ON s.uid=p.uid WHERE s.uid>0 AND invisible=0 AND action IN (2,127) AND fid=0 AND tid=0");
	if(!empty($config['onlinebot'])){
		$re2 = DB::query("SELECT s.uid,s.username,s.groupid,g.color,n.point_total".($config['chat_point']!='9'?",p.extcredits{$config['chat_point']} AS point":"")." FROM ".DB::table('common_member')." s LEFT JOIN ".DB::table('common_usergroup')." g ON s.groupid=g.groupid LEFT JOIN ".DB::table('newz_nick')." n ON s.uid=n.uid LEFT JOIN ".DB::table('common_member_count')." p ON s.uid=p.uid WHERE s.uid IN (".$config['onlinebot'].")");
	}
}else{
	$re = DB::query("SELECT s.uid,s.username,s.groupid,s.lastactivity,g.color,n.point_total FROM ".DB::table('common_session')." s LEFT JOIN ".DB::table('common_usergroup')." g ON s.groupid=g.groupid LEFT JOIN ".DB::table('newz_nick')." n ON s.uid=n.uid WHERE s.uid>0 AND invisible=0 AND action IN (2,127) AND fid=0 AND tid=0");
	if(!empty($config['onlinebot'])){
		$re2 = DB::query("SELECT s.uid,s.username,s.groupid,g.color,n.point_total FROM ".DB::table('common_member')." s LEFT JOIN ".DB::table('common_usergroup')." g ON s.groupid=g.groupid LEFT JOIN ".DB::table('newz_nick')." n ON s.uid=n.uid WHERE s.uid IN (".$config['onlinebot'].")");
	}
}
...
while($r = DB::fetch($re) OR $r = DB::fetch($re2)){
	...
	if($time-$r['lastactivity']>$timeout2)
	{
		...
		<td><a '.($r['color']?'style="background-color:'.$r['color'].'" ':'').'class="nznametop2" id="nzolpro_'.$r['uid'].'" target="_blank" style="margin-top: 8px;">'.$r['username'].'</a></td>';
	}
