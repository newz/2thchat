...
while ($r = DB::fetch($re) or $r = DB::fetch($re2)) {
    ...
    if ($time - $r['lastactivity'] > $timeout) {
        ...
        $body_onlinein[$r['groupid']] .= '<div class="nzolcon"><div class="nzolname">
		...
		</div><div style="display:inline-block;margin-left:10px;position:relative;top:-4px;height:30px;"><a class="nznametop2" id="nzolpro_' . $r['uid'] . '" onclick="nzAt(\'' . $r['username'] . '\');" style="cursor:pointer'.($r['color']?';color:'.$r['color']:'').'">' . $r['username'] . '</a><br>
		<span style="line-height: 15px;">'.$r['grouptitle'].'</span></div>
		</div></div>';
    }
    $oltotal++;
}
