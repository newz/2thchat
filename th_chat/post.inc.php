...
if(substr($text,0,4)=="/ban"&&$is_mod){
	...
	$banned[] = $uid_ban;
	$username_ban = DB::query("SELECT m.username AS name,n.name AS nick FROM ".DB::table('common_member')." m LEFT JOIN 2thchat_nick n ON m.uid=n.uid WHERE m.uid='{$uid_ban}' LIMIT 1");
...
$re = DB::query("SELECT uid,point_time FROM 2thchat_nick WHERE uid='{$uid}'");
...
if($re = DB::fetch($re)){
	...
	if($time-$re['point_time']<10){
		...
		DB::query("UPDATE 2thchat_nick SET point_time='{$time}' WHERE uid='{$uid}' LIMIT 1");	
	}
	
}else{
	
	DB::query("INSERT INTO 2thchat_nick (uid,point_time) VALUES ('{$uid}','{$time}')");
...
$username_point = DB::query("SELECT m.username AS name,n.name AS nick,p.extcredits{$config['chat_point']} AS point FROM ".DB::table('common_member')." m LEFT JOIN 2thchat_nick n ON m.uid=n.uid LEFT JOIN ".DB::table('common_member_count')." p ON m.uid=p.uid WHERE m.uid='{$uid_point}' LIMIT 1");
...
$text = paddslashes(discuzcode($text,0,0));
...
$text = 'ล้างข้อมูล <span id="del_'.$seedd.'"></span><script type="text/javascript">nzchatobj("#del_'.$seedd.'").parent().parent().parent().parent().nextUntil().remove();</script>';
}

DB::query("INSERT INTO 2thchat_data (uid,touid,text,time,ip) VALUES ('{$uid}','{$touid}','{$text}','".time()."','{$ip}')");

$last = DB::insert_id();

DB::query("DELETE FROM 2thchat_data WHERE id<".($last-500));
...
if ($c['ip'] == 'changename'){
	...
	$body[$c['id']]  .= chatrow($c['id'],$c['text'],$c['uid'],$c['nick'],$c['time'],$c['color'],$c['touid'],1);
