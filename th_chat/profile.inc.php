...
$r = DB::fetch_first("SELECT s.uid,s.username,s.groupid,g.grouptitle,g.color,n.point_total,p.extcredits1,p.extcredits2,p.extcredits3,p.extcredits4,p.extcredits5,p.extcredits6,p.extcredits7,p.extcredits8 FROM ".DB::table('common_member')." s LEFT JOIN ".DB::table('common_usergroup')." g ON s.groupid=g.groupid LEFT JOIN ".DB::table('newz_nick')." n ON s.uid=n.uid LEFT JOIN ".DB::table('common_member_count')." p ON s.uid=p.uid WHERE s.uid=".$uid);
if($uid==$_G['uid']){
